...
namespace Comm.Network.Iocp
{
    public class Session<C>
    {
        ...
        public CallBack<Session<C>, SocketAsyncEventArgs, byte, byte[], ushort, ushort> Handle;
        private ServerIOCP<C> server;
        private SocketAsyncEventArgs recv;
        public static Pools<SocketAsyncEventArgs> sendEventArgsPool;

        public void Init(ServerIOCP<C> _server, SocketAsyncEventArgs _recv, int _bufferSize, CallBack<Session<C>, SocketAsyncEventArgs, byte, byte[], ushort, ushort> _Handle)
        {
            server = _server;
            recv = _recv;
        ...
        private void ProcessReceive()
        {
            ...
            if (recv.BytesTransferred > 0)
            {
                if (recv.SocketError == SocketError.Success)
                {
                    ...
                    {//接受的数据超过缓冲区
                        Comm.Util.Log.Debug("====接受的数据超过缓冲区====");
        ...
        private void ProcessSendError(SocketAsyncEventArgs sendEventArgs)
        {
            ...
            Comm.Util.Log.Error("[Server] Socket error {0} on endpoint {1} during {2}.", (Int32)sendEventArgs.SocketError, localEndPoint, sendEventArgs.LastOperation);
            CloseSocket(socket);
